...
# Reads source data from data/figures/figureSupplement5_source_data.csv
...
source_data <- read.csv(here::here("data/figures/figureSupplement5_source_data.csv"), stringsAsFactors = FALSE)

# Re-apply factor orderings
source_data$stim      <- factor(source_data$stim, levels = c("Pre-Stim","Post-Stim"))
source_data$nt        <- factor(source_data$nt, levels = c("DA","SE","NE"))
source_data$idx       <- factor(source_data$idx)

# Panels A/B: trial-level data
rl.trial.plot <- source_data %>% filter(panel == "A")
ug.trial.plot <- source_data %>% filter(panel == "B")

# Panels C/D: subject averages
rl.avg.plot <- source_data %>% filter(panel == "C")
ug.avg.plot <- source_data %>% filter(panel == "D")

# Panel E: RL by block type
rl.task.plot <- source_data %>% filter(panel == "E") %>%
  mutate(block_type = factor(condition, levels = c("Reward","Mixed","Punishment")),
...
# Panel F: UG by offer bin
ug.task.plot <- source_data %>% filter(panel == "F") %>%
  mutate(offer_bin = factor(condition, levels = c("$1-3","$4-6","$7-9")),
...
# Build offset data for subject-average panels (C/D)
offset_data_RL <- rl.avg.plot %>%
  mutate(
...
fig.ug.avg =
  ...
  stat_summary(data = offset_data_UG,aes(x = box_x_pos,y = Oz, group = stim, color = color),
...
fig.ug.task =
  ...
  facet_wrap(~nt) +
  geom_hline(yintercept = 0, linetype = "dashed", linewidth = 1, color = "gray") +
  geom_point(data = ug.task.plot,
    aes(x = point_x_pos, y = Oz, group = idx, color = color),
    shape = 1,
    stroke = 1.75,
    alpha = 0.75,
    size = 1.5) +
  stat_summary(data = ug.task.plot,
...
ggsave(here::here("results/from_source_data/figureS5.png"),
       plot = final_plot,
       device = "png",
