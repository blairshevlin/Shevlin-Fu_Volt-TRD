...
# 2025/11/10    Blair Shevlin                           shared z-scoring procedure
...
rl.EST.Shared= 
  tasks.EST.alt %>% filter(task == "RL", event == "Shared") %>%
...
ug.EST.Shared = 
  tasks.EST.alt %>% filter(task == "UG",event == "Shared") %>%
  group_by(idx,stim,nt) %>%
  mutate(stim = factor(stim, levels = c("Pre-Stim","Post-Stim")),
  trial_z = scale(trial)[,1]) %>%
  ungroup()
...
EST.Shared = rbind(
  rl.EST.Shared %>% mutate(task = "RL"),
  ug.EST.Shared %>% mutate(task = "UG")
)  %>% mutate(task = factor(task, levels = c("RL","UG")))


# IDs
ids_final = unique(ug.EST.Shared$idx)
...
# Add details
rl.OR = rl.beh %>% filter(sess %in% c("Pre-Stim","Post-Stim")) %>%
  mutate(stim = factor(sess, levels = c("Pre-Stim","Post-Stim"))) %>%
  group_by(idx,sess,cond) %>%
  mutate(prev_rew_raw =  lag(outcome,1)) %>%  
  ungroup()  

ug.OR = ug.beh %>% filter(sess %in% c("Pre-Stim","Post-Stim")) %>%
  mutate(stim = factor(sess, levels = c("Pre-Stim","Post-Stim"))) %>%
    group_by(idx,sess) %>%
    mutate(prev_offer_raw =  lag(offer,1), 
            offer_bin = ifelse(offer < 4, "Low", ifelse(offer > 6, "High", "Middle"))) %>%  
    ungroup()  

rl.EST.Reward= merge(rl.EST.Shared, rl.OR, by = c("idx","stim","trial","rt"))
ug.EST.Offer = merge(ug.EST.Shared, ug.OR, by = c("idx","stim","trial","rt"))
...
summary(rl.DA.anova) # Significant stim effect
summary(rl.SE.anova) # NS 

summary(ug.DA.anova) # NS
summary(ug.SE.anova) # Significant sitm effect

# Combined anova
rl.EST.aov.combo = rl.EST.pr %>% group_by(idx,stim,nt) %>%
  summarise(oz = mean(Oz))
ug.EST.aov.combo = ug.EST.pr %>% group_by(idx,stim,nt) %>%
  summarise(oz = mean(Oz))

EST.aov.combo = rbind(
  rl.EST.aov.combo %>% mutate(task = "RL"),
  ug.EST.aov.combo %>% mutate(task = "UG")
)  %>% mutate(task = factor(task, levels = c("RL","UG")))

DA.anova=aov(oz ~ stim * task,
                data = EST.aov.combo[EST.aov.combo$nt == "DA",])
SE.anova=aov(oz ~ stim * task,
                data = EST.aov.combo[EST.aov.combo$nt == "SE",])

summary(DA.anova) # Significant stim effect and a significant task effect
summary(SE.anova) # Significant stim effect, but not task specific  
...
sess.lm.DA = lmer(Oz ~ stim * task * trial_z +
                    (1+stim * task|idx),
               data = EST.Shared[EST.Shared$nt == "DA",],
               REML=F,
               control = lmerControl(optimizer = "bobyqa"))

sess.lm.SE = lmer(Oz ~ stim * task * trial_z +
                    (1+stim * task|idx),
                  data = EST.Shared[EST.Shared$nt == "SE",],
                  REML=F,
                  control = lmerControl(optimizer = "bobyqa"))

sess.lm.NE = lmer(Oz ~ stim * task * trial_z +
                    (1+stim * task|idx),
                  data = EST.Shared[EST.Shared$nt == "NE",],
                  REML=F,
                  control = lmerControl(optimizer = "bobyqa"))
...
da_slopes <- emmeans(sess.lm.DA, ~ stim * task)
se_slopes <- emmeans(sess.lm.SE, ~ stim * task)
ne_slopes <- emmeans(sess.lm.NE, ~ stim * task)

pairs(da_slopes, by = "stim")
pairs(se_slopes, by = "stim")
pairs(ne_slopes, by = "stim")

pairs(da_slopes, by = "task")
pairs(se_slopes, by = "task")
